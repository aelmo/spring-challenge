...
public class CrudCategoryController {
    ...
    public CrudCategoryController(final CategoryRepository categoryRepository, final CategoryService categoryService) {
    ...
    public ResponseEntity<?> createNewCategory(@ApiParam(value = "Object for new Category", required = true)
        ...
        try {
            ...
            return new ResponseEntity<>(new UserResponseSaveDTO(true, "Category created with success!", createdCategory.getId()), HttpStatus.CREATED);
        } catch (Exception e) {
            e.printStackTrace();
